...
public abstract class BlockEngineBase extends BlockBuildCraft implements ICustomHighlight {
    ...
    public void randomDisplayTick(World world, int i, int j, int k, Random random) {
        ...
        if (((TileEngineBase) tile).getEnergyStage() == TileEngineBase.EnergyStage.OVERHEAT) {
            for (int f = 0; f < 16; f++) {
                world.spawnParticle("smoke", i + 0.4F + (random.nextFloat() * 0.2F), j + (random.nextFloat() * 0.5F), k + 0.4F + (random.nextFloat()
                    * 0.2F), random.nextFloat() * 0.04F - 0.02F, random.nextFloat() * 0.05F + 0.02F, random.nextFloat() * 0.04F - 0.02F);
            }
