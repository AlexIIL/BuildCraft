...
public final class BlockUtils {
    ...
    public static List<ItemStack> getItemStackFromBlock(WorldServer world, int i, int j, int k) {
        ...
        float dropChance = ForgeEventFactory.fireBlockHarvesting(dropsList, world, block, i, j, k, meta, 0, 1.0F, false, CoreProxy.proxy
                .getBuildCraftPlayer(world).get());
    ...
    public static boolean harvestBlock(WorldServer world, int x, int y, int z, ItemStack tool) {
        BreakEvent breakEvent = new BreakEvent(x, y, z, world, world.getBlock(x, y, z), world.getBlockMetadata(x, y, z), CoreProxy.proxy
                .getBuildCraftPlayer(world).get());
    ...
    public static void explodeBlock(World world, int x, int y, int z) {
        ...
        for (EntityPlayer player : (List<EntityPlayer>) world.playerEntities) {
            ...
            if (player.getDistanceSq(x, y, z) < 4096) {
                ((EntityPlayerMP) player).playerNetServerHandler.sendPacket(new S27PacketExplosion(x + .5, y + .5, z + .5, 3f,
    ...
    public static int computeBlockBreakEnergy(World world, int x, int y, int z) {
        return (int) Math.floor(BuilderAPI.BREAK_ENERGY * BuildCraftCore.miningMultiplier * ((world.getBlock(x, y, z).getBlockHardness(world, x, y, z)
            + 1) * 2));
    }
