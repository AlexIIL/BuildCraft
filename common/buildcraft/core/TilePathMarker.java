...
public class TilePathMarker extends TileMarker implements IPathProvider {
    ...
    public void createLaserAndConnect(TilePathMarker pathMarker) {
        ...
        LaserData laser = new LaserData(new Position(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5), new Position(pathMarker.xCoord + 0.5,
    ...
    private TilePathMarker findNearestAvailablePathMarker() {
        ...
        for (TilePathMarker t : availableMarkers) {
            if (t == this || t == this.links[0] || t == this.links[1] || t.getWorldObj().provider.dimensionId != this
    ...
    public ArrayList<BlockIndex> getPath() {
        ...
        while (nextTile != null) {
            ...
            if (nextTile.links[0] != null && !visitedPaths.contains(new BlockIndex(nextTile.links[0].xCoord, nextTile.links[0].yCoord,
                    nextTile.links[0].zCoord))) {
                nextTile = nextTile.links[0];
            } else if (nextTile.links[1] != null && !visitedPaths.contains(new BlockIndex(nextTile.links[1].xCoord, nextTile.links[1].yCoord,
