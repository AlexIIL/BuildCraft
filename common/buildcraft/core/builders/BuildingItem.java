...
public class BuildingItem implements IBuildingItem, ISerializable {
    ...
    public void initialize() {
        if (!initialized) {
            ...
            double d2 = Math.sqrt(destinationToTop.x * destinationToTop.x + destinationToTop.y * destinationToTop.y + destinationToTop.z
                * destinationToTop.z);
    ...
    private void build() {
        if (slotToBuild != null) {
            ...
            if (slotToBuild.writeToWorld(context)) {
                context.world().playAuxSFXAtEntity(null, 2001, destX, destY, destZ, Block.getIdFromBlock(oldBlock) + (oldMeta << 12));
            } else {
