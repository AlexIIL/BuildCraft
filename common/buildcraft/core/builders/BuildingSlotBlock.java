            ...
            try {
                ...
                if (!getSchematic().isAlreadyBuilt(context, x, y, z)) {
                    if (context.world().isAirBlock(x, y, z)) {
                        return false;
                    } else if (!(getSchematic() instanceof SchematicBlock) || context.world().getBlock(x, y, z).isAssociatedBlock(
                            ((SchematicBlock) getSchematic()).block)) {
                        BCLog.logger.warn(
                                "Placed block does not match expectations! Most likely a bug in BuildCraft or a supported mod. Removed mismatched block.");
                        BCLog.logger.warn("Location: " + x + ", " + y + ", " + z + " - Block: " + Block.blockRegistry.getNameForObject(context.world()
                                .getBlock(x, y, z)) + "@" + context.world().getBlockMetadata(x, y, z));
