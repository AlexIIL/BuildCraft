...
public class TileQuarry extends TileAbstractBuilder implements IHasWork, ISidedInventory, IDropControlInventory, IPipeConnection, IControllable,
    ...
    public boolean areChunksLoaded() {
        ...
        return worldObj.blockExists(box.xMin, box.yMax, box.zMin) && worldObj.blockExists(box.xMax, box.yMax, box.zMin) && worldObj.blockExists(
                box.xMin, box.yMax, box.zMax) && worldObj.blockExists(box.xMax, box.yMax, box.zMax);
    }
    ...
    protected int getNetworkUpdateRange() {
        return DefaultProps.NETWORK_UPDATE_RANGE + (int) Math.ceil(Math.sqrt(yCoord * yCoord + box.sizeX() * box.sizeX() + box.sizeZ() * box
                .sizeZ()));
    }
    ...
    private void initializeBlueprintBuilder() {
        Blueprint bpt = ((FillerPattern) FillerManager.registry.getPattern("buildcraft:frame")).getBlueprint(box, worldObj,
                new IStatementParameter[0], BuildCraftBuilders.frameBlock, 0);
    ...
    public void writeData(ByteBuf stream) {
        ...
        ledState = (hasWork() && mode != Mode.Off && getTicksSinceEnergyReceived() < 12 ? 16 : 0) | (getBattery().getEnergyStored() * 15
    ...
    public void forceChunkLoading(Ticket ticket) {
        ...
        if (placedBy != null && !(placedBy instanceof FakePlayer)) {
            placedBy.addChatMessage(new ChatComponentTranslation("chat.buildcraft.quarry.chunkloadInfo", xCoord, yCoord, zCoord, chunks.size()));
        }
    }
