...
public class AIRobotGotoBlock extends AIRobotGoto {
    ...
    public void update() {
        if (path == null && pathSearch == null) {
            pathSearch = new PathFinding(robot.worldObj, new BlockIndex((int) Math.floor(robot.posX), (int) Math.floor(robot.posY), (int) Math.floor(
                    robot.posZ)), new BlockIndex((int) Math.floor(finalX), (int) Math.floor(finalY), (int) Math.floor(finalZ)), maxDistance, 96);
