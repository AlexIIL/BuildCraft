    ...
    public void update() {
        ...
        if (blockDamage > 1.0F) {
            ...
            if (robot.getHeldItem() != null) {
                if (robot.getHeldItem().getItem().onBlockStartBreak(robot.getHeldItem(), blockToBreak.x, blockToBreak.y, blockToBreak.z,
                        CoreProxy.proxy.getBuildCraftPlayer((WorldServer) robot.worldObj).get())) {
                    continueBreaking = false;
                }
            }

            if (continueBreaking && BlockUtils.harvestBlock((WorldServer) robot.worldObj, blockToBreak.x, blockToBreak.y, blockToBreak.z, robot
                    .getHeldItem())) {
