...
public class RobotStationPluggable extends PipePluggable implements IPipePluggableItem, IEnergyReceiver, IDebuggable, IDockingStationProvider {
    public class RobotStationPluggableRenderer implements IPipePluggableRenderer {
        ...
        public void renderPluggable(RenderBlocks renderblocks, IPipe pipe, ForgeDirection side, PipePluggable pipePluggable,
            ...
            switch (state) {
                case None:
                case Available:
                    blockStateMachine.getTextureState().set(BuildCraftTransport.instance.pipeIconProvider.getIcon(
    ...
    public void validate(IPipeTile pipe, ForgeDirection direction) {
        if (!isValid && !pipe.getWorld().isRemote) {
            station = (DockingStationPipe) RobotManager.registryProvider.getRegistry(pipe.getWorld()).getStation(pipe.x(), pipe.y(), pipe.z(),
                    direction);
