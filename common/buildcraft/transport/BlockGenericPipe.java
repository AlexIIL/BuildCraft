...
public class BlockGenericPipe extends BlockBuildCraft implements IColorRemovable {
    ...
    private boolean addOrStripPipePluggable(World world, int x, int y, int z, ItemStack stack, EntityPlayer player, ForgeDirection side,
            Pipe<?> pipe) {
        RaytraceResult rayTraceResult = doRayTrace(world, x, y, z, player);
    ...
    public static boolean placePipe(Pipe<?> pipe, World world, int i, int j, int k, Block block, int meta, EntityPlayer player, ForgeDirection side) {
        ...
        if (player != null) {
            ...
            BlockEvent.PlaceEvent placeEvent = new BlockEvent.PlaceEvent(new BlockSnapshot(world, i, j, k, block, meta), placedAgainst, player);
        ...
        for (int i = 0; i < its; ++i) {
            for (int j = 0; j < its; ++j) {
                for (int k = 0; k < its; ++k) {
                    ...
                    EntityDiggingFX fx = new EntityDiggingFX(worldObj, px, py, pz, px - x - 0.5D, py - y - 0.5D, pz - z - 0.5D,
                            BuildCraftTransport.genericPipeBlock, random, meta);
                    fx.setParticleIcon(icon);
