...
public class PipeTransportItems extends PipeTransport implements IDebuggable {
    ...
    private void moveSolids() {
        ...
        for (TravelingItem item : items) {
            ...
            if ((item.toCenter && middleReached(item)) || outOfBounds(item)) {
                ...
                item.setPosition(container.xCoord + 0.5, container.yCoord + TransportUtils.getPipeFloorOf(item.getItemStack()), container.zCoord
                    + 0.5);
    ...
    protected boolean middleReached(TravelingItem item) {
        ...
        return Math.abs(container.xCoord + 0.5 - item.xCoord) < middleLimit && Math.abs(container.yCoord + TransportUtils.getPipeFloorOf(item
                .getItemStack()) - item.yCoord) < middleLimit && Math.abs(container.zCoord + 0.5 - item.zCoord) < middleLimit;
    }
